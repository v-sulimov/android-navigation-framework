...
object Logger {
     ...
     */
    fun logScreenFragmentCreated(fragment: Fragment, typeId: String) {
     ...
     */
    fun logOverlayFragmentCreated(dialogFragment: DialogFragment, typeId: String) {
        Log.d(TAG, "Overlay dialog fragment ${dialogFragment::class.simpleName} successfully created for overlay type $typeId.")
    }
     ...
     */
    fun logOverlayStateExistsButDialogFragmentMismatch(newTypeId: String, currentTypeId: String?) {
        Log.d(TAG, "Overlay state for $newTypeId exists, but the current DialogFragment is for $currentTypeId. Replacing with the correct DialogFragment.")
    }
}
