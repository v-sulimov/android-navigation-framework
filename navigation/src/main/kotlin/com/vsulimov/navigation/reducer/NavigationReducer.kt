 ...
 */
class NavigationReducer<Screen, Overlay> : TypedReducer<NavigationAction, NavigationState<Screen, Overlay>>(
    ...
    override fun reduceTyped(
        ...
        return when (action) {
            is NavigationAction.NavigateTo -> {
                try {
                    val backStack =
                        if (action.clearBackStack) CopyOnWriteStack<Screen>() else state.backStack.push(state.screen)
