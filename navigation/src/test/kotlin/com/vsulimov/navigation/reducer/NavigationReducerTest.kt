...
class NavigationReducerTest {
    ...
    sealed class ScreenState(override val typeId: String) : NavigationComponent {
        
        data class Home(override val typeId: String = "Home") : ScreenState(typeId)
        
        data class Details(override val typeId: String = "Details") : ScreenState(typeId)

        data class Current(override val typeId: String = "Current") : ScreenState(typeId)

        data class Previous(override val typeId: String = "Previous") : ScreenState(typeId)

        data class A(override val typeId: String = "A") : ScreenState(typeId)

        data class B(override val typeId: String = "B") : ScreenState(typeId)

        data class C(override val typeId: String = "C") : ScreenState(typeId)
    }

    sealed class OverlayState(override val typeId: String) : NavigationComponent {

        data class Default(override val typeId: String = "Default") : OverlayState(typeId)
    }
    ...
    fun `NavigateTo pushes current screen to back stack and sets new screen`() {
        val initialState = NavigationState<ScreenState, OverlayState>(
    ...
    fun `Multiple NavigateTo and GoBack actions handle back stack correctly`() {
        var state = NavigationState<ScreenState, OverlayState>(
